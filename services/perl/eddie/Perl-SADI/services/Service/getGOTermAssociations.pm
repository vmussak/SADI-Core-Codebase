#-----------------------------------------------------------------
# Service name: getGOTermAssociations
# Authority:    illuminae.com
# Created:      24-Feb-2010 14:34:08 PST
# Contact:      myaddress@organization.org
# Description:
#               gets the proteins annotated with a given GO term
#-----------------------------------------------------------------
package Service::getGOTermAssociations;
use FindBin qw( $Bin );
use lib $Bin;
...
BEGIN {
	...
	new SADI::Generators::GenServices->async_load(
								   service_names => ['getGOTermAssociations'] );
}

# (this to stay here with any of the options above)
use vars qw( @ISA );
@ISA = qw( com::illuminae::getGOTermAssociationsBase );
use strict;
...
sub process_it {
	...
	foreach my $input (@inputs) {
		...
		my $class = Implementations::getGOTermAssociations($input, $id, $LOG);
...
Service::getGOTermAssociations - a SADI service
