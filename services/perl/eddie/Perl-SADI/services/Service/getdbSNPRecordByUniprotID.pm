...
# Created:      05-Feb-2010 10:52:31 PST
# Contact:      edward.kawas+sadi@gmail.com
# Description:
#               This service retrieves dbSNP record IDs by Uniprot IDs
#-----------------------------------------------------------------
package Service::getdbSNPRecordByUniprotID;
use FindBin qw( $Bin );
use lib $Bin;
...
BEGIN {
	...
	new SADI::Generators::GenServices->async_load(
							   service_names => ['getdbSNPRecordByUniprotID'] );
}

# (this to stay here with any of the options above)
use vars qw( @ISA );
@ISA = qw( net::biordf::dev::getdbSNPRecordByUniprotIDBase );
use strict;
...
sub process_it {
	...
	foreach my $input (@inputs) {
        ...
        unless ($accession) {
			...
			# extract the uniprot record id
			if ( $accession =~ m/uniprot:(\S+)$/gi ) {
				$accession = $1;
			} else {
				next;
			}
        }
		my $class = Implementations::getdbSNPRecordByUniprotID($input, $accession, $LOG);
...
Service::getdbSNPRecordByUniprotID - a SADI service
...
B<Email>: edward.kawas+sadi@gmail.com
