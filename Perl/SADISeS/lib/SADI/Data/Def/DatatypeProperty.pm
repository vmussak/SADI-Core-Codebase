#-----------------------------------------------------------------
# SADI::Data::Def::DatatypeProperty
		...
		parent => {
			...
			post => sub {
				...
				$self->{module_parent} =
				  ...
				  unless $self->{parent} eq 'SADI::Data::OWL::DatatypeProperty';
				$self->{module_parent} = $self->{parent}
				  if $self->{parent} eq 'SADI::Data::OWL::DatatypeProperty';
			},
		},
...
sub init {
	my ($self) = shift;
	$self->SUPER::init();
	$self->parent('SADI::Data::OWL::DatatypeProperty');
}
1;
__END__
