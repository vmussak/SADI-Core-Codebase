#-----------------------------------------------------------------
# Service name: getGOTermDefinitions
# Authority:    dev.biordf.net
# Created:      09-Feb-2010 07:35:58 PST
# Contact:      edward.kawas+sadi@gmail.com
# Description:
#               This service fetches GO definitions given a GO record ID
#-----------------------------------------------------------------
package Service::getGOTermDefinitions;
use FindBin qw( $Bin );
use lib $Bin;
...
BEGIN {
	...
	new SADI::Generators::GenServices->async_load(
									service_names => ['getGOTermDefinitions'] );
}

# (this to stay here with any of the options above)
use vars qw( @ISA );
@ISA = qw( net::biordf::dev::getGOTermDefinitionsBase );
use strict;
...
sub process_it {
	...
	foreach my $input (@inputs) {
		...
		my $class = Implementations::getGOTermDefinitions($input, $id, $LOG);
...
Service::getGOTermDefinitions - a SADI service
